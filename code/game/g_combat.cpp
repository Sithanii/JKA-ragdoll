...
{
	if (g_useRagdoll->integer && !self->physRagdoll) {
		self->physRagdoll = new SimpleRagdoll(self);
		vec3_t force;
		float forceMagnitude = damage * g_ragdollForce->value;

		switch (meansOfDeath) {
		case MOD_SABER: {
			if (attacker && attacker->client) {
				AngleVectors(attacker->client->ps.viewangles, force, NULL, NULL);
				forceMagnitude *= 2.0f;
			}
			break;
		}
		...
		case MOD_BRYAR: {
			if (inflictor) {
				VectorSubtract(self->s.origin, inflictor->s.origin, force);
				forceMagnitude *= 0.7f;
			}
			break;
		}
		...
		VectorScale(force, forceMagnitude, force);

		vec3_t randomForce;
		VectorSet(randomForce,
			Q_flrand(-0.2f, 0.2f),
			Q_flrand(-0.2f, 0.2f),
			Q_flrand(0.0f, 0.3f));
		VectorAdd(force, randomForce, force);
